...
magnetic_field_strengths = np.linspace(-0.8, 0.8, 4, endpoint=True)
solenoid_lengths = np.linspace(0.01, 0.10, 4, endpoint=True)
solenoid_positions = np.linspace(0.09, 0.20, 4, endpoint=True)
...
def do_scan(i, path, magnetic_field_strength, solenoid_length, solenoid_position):
    ...
    s.run()
    s.readData()
    result = analyze(s.data, path, 0.12, 0.4, s.parametersDict())
    s.deleteData()
    return result
...
def analyze(data, path, end_of_gun, end_of_drift, parameters_dict):
    ...
    return emit_6d[waist_index]

def main():
    global idx
    idx = 0
    dir = pathlib.Path('/a/chansel/mot/optimize')
    with open('/home/chansel/mot/optimization_log.txt', 'w+') as f:
        def optimize_me(args):
            global idx
            result = do_scan(idx, dir / str(idx), *args)
            idx += 1
            f.write(f"evaluation {idx}: {args} -> {result}\n")
            f.flush()
            return result
        result = scipy.optimize.dual_annealing(optimize_me, ((-0.8, 0.8), (0.01, 0.1), (0.09, 0.2)))

if __name__ == '__main__':
    mot.detach_run(__file__, main)
