...
def main():
    s_ions = mot.MotGunSimulation(f'/a/chansel/mot/ions/0')
    s_ions.readData()
    s_no_ions = mot.MotGunSimulation(f'/a/chansel/mot/ions/1')
    s_no_ions.readData()
    os.system('mkdir -p ions.py.results')
    analyze(s_ions.data, s_no_ions.data, 'Ions On', 'Ions Off', 0.12, 'ions.py.results')
    analyze2(s_ions.data, s_no_ions.data, 'Ions On', 'Ions Off', 0.12, 'ions.py.results')
    analyze3(s_ions.data, s_no_ions.data, 'Ions On', 'Ions Off', 0.12, 'ions.py.results')
    os.system('rm -rf ions.py.results/frames')
    os.system('tar -czvf ions.py.results.tgz ions.py.results')

def run_simulation(i):
    print(f'running simulation {i}')
    s = mot.MotGunSimulation(f'/a/chansel/mot/ions/{i}')
    s.sigma_r = 0.001
    ...
    s.sol_length = 0.1
    s.sol_z = 0.1
    s.ions_on = scan_parameters[i]
    s.spacecharge = 'approximate'
    s.initializeParticles(seed=657835)
    s.run()
    s.readData()
    s.analyze()
    return i
